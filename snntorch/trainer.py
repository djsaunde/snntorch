...
class DistributedSNNTrainer:
    ...
    def forward_pass(self, data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        context = torch.cuda.amp.autocast() if self.mixed_precision else nullcontext()
        ...
        spike_tensor = torch.stack(self.spike_recordings) if self.spike_recordings else None
        mem_tensor = torch.stack(self.mem_recordings) if self.mem_recordings else None
        
        return spike_tensor, mem_tensor
    
    def compute_loss(self, 
                     spike_data: torch.Tensor, 
                     mem_data: Optional[torch.Tensor], 
                     targets: torch.Tensor, 
                     loss_fn: Callable,
                     loss_mode: str = "sum_spikes") -> torch.Tensor:
        """
    ...
    def train_step(self, 
                   ...
                   loss_fn: Callable,
                   loss_mode: str = "sum_spikes") -> Dict[str, float]:
        """
        ...
        spike_data, mem_data = self.forward_pass(data)
        forward_time = time.time() - forward_start
        
        # Calculate loss
        loss = self.compute_loss(spike_data, mem_data, targets, loss_fn, loss_mode)
        ...
        return {
            'loss': loss.item() * self.grad_accumulation_steps,  # Unscale for reporting
    ...
    def train_epoch(self, 
        ...
        for batch_idx, (data, targets) in enumerate(dataloader):
            ...
            # Training step
            step_stats = self.train_step(data, targets, optimizer, loss_fn, loss_mode)
    ...
    def load_checkpoint(self, 
        ...
        additional_info = {k: v for k, v in checkpoint.items() 
                          if k not in ['epoch', 'global_step', 'model_state_dict', 
                                      'optimizer_state_dict', 'training_stats', 'scheduler_state_dict']}
...
def get_optimizer_and_scheduler(model: nn.Module,
                               ...
                               **kwargs) -> Tuple[torch.optim.Optimizer, Optional[torch.optim.lr_scheduler._LRScheduler]]:
    """
