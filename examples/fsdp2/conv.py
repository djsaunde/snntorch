...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Number of hidden units in FC layers (default: 128)",
    )
    parser.add_argument(
        "--num-outputs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Number of time steps (default: 25)",
    )
...
class SNNModel(nn.Module):
    """Convolutional SNN model with proper time stepping and state management."""
    ...
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            16, 32, kernel_size=5, stride=1, padding=2
        )  # 14x14 -> 14x14
        self.lif2 = snn.Leaky(
            beta=beta,
            spike_grad=surrogate.fast_sigmoid(),
        ...
        self.fc1 = nn.Linear(32 * 7 * 7, num_hidden)
        self.lif3 = snn.Leaky(
            beta=beta,
            spike_grad=surrogate.fast_sigmoid(),
            reset_mechanism="zero",
    ...
    def forward(self, x):
        ...
        """
        # Reshape flattened input back to 2D image (batch, 784) -> (batch, 1, 28, 28)
...
def create_mnist_spike_data(num_steps=25, val_split=0.0, use_val=False):
...
def main():
    ...
    # Setup training mode (FSDP2, DDP, or single-GPU)
    device, is_distributed, _ = setup_training_mode(args)
    ...
    step_losses = []
    step_accuracies = []
    spike_data_recorded = None
    mem_data_recorded = None
    input_data_recorded = None
